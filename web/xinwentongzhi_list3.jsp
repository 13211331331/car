...
%>
      <tr>
		...
		{
		%>
            <a href='<%=shouyetupian %>' target='_blank'>点此下载</a>
            <%
		}
	%>
        </td>
  ...
  <br>
  <p align="center">以上数据共<%=i %>条&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="xinwentongzhi_list3.jsp?page=1&lb=<%=lb%>">首页</a>　<a href="xinwentongzhi_list3.jsp?page=<%= curpage-1%>&lb=<%=lb%>">上一页</a>　<A href="xinwentongzhi_list3.jsp?page=<%= curpage+1%>&lb=<%=lb%>">下一页</A>　<a href="xinwentongzhi_list3.jsp?page=<%=zys %>&lb=<%=lb%>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页
